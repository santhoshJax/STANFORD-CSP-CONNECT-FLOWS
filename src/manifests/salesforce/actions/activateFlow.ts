/**
 * This file was automatically generated by spectral.
 * Any changes made will be overwritten.
 */
import type { Connection } from "@prismatic-io/spectral/dist/types";
import { requireContext } from "@prismatic-io/spectral/dist/serverTypes";

export interface ActivateFlowValues {
  /**
   * Version
   *
   * @default 63.0
   */
  version?: string;
  /**
   * Flow Name
   *
   */
  flowName: string;
  /**
   * Connection
   *
   */
  connection: Connection;
}

/**
 * Activate Flow
 *
 * @description Activate a Flow in Salesforce by name.
 */
export const activateFlow = {
  key: "activateFlow",
  perform: async <TReturn>(values: ActivateFlowValues): Promise<TReturn> => {
    const context = requireContext();
    return (await context.components.salesforce.activateFlow({
      ...values,
    })) as TReturn;
  },
  inputs: {
    version: {
      inputType: "string",
      collection: undefined,
      default: `63.0`,
    },
    flowName: {
      inputType: "string",
      collection: undefined,
      default: ``,
      required: true,
    },
    connection: {
      inputType: "connection",
      collection: undefined,
      default: `null`,
      required: true,
    },
  },
  examplePayload: { data: { errors: [], success: true, fullName: "My_Flow" } },
} as const;
